...
export type AppRole =
  'Dispatcher' | 'AmbulanceDriver' | 'TriageNurse' | 'DischargeClerk' | 'SystemAdmin';
...
export class AuthService {
  ...
  get token(): string | null {
    return localStorage.getItem('amc_access_token');
  }
  ...
  private readUser(): AuthUser | null {
    try {
      return JSON.parse(localStorage.getItem('amc_user') || 'null');
    } catch {
      return null;
    }
  }
